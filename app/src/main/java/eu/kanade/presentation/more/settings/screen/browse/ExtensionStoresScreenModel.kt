...
class ExtensionStoresScreenModel(
                ...
                .collectLatest { stores ->
                    ...
                    mutableState.update {
                        when (it) {
                            ...
                            is ExtensionStoreScreenState.Success -> it.copy(
                                stores = stores,
                                // KMK -->
                                disabledRepos = disabledRepos,
                                // KMK <--
    ...
    // KMK -->
    /**
     * Enables a previously disabled repo.
     */
    fun enableRepo(indexUrl: String) {
        sourcePreferences.disabledRepos.getAndSet { it - indexUrl }
        updateSuccessState { it.copy(disabledRepos = sourcePreferences.disabledRepos.get()) }
        screenModelScope.launchIO {
            extensionManager.findAvailableExtensions()
        }
    }
    ...
    /**
     * Disables a repo, hiding its extensions from the available list without removing it.
     */
