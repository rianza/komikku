...
object ImageUtil {
    ...
    fun addHorizontalCenterMargin(
        ...
        val imageBitmap = decodeBitmap(imageSource.inputStream())!!
        val height = imageBitmap.height
        val width = imageBitmap.width
    ...
    // SY -->
    /**
     * Decodes an image stream into a full-size software bitmap using the libvips-based
     * decoder (successor of the legacy tachiyomi.decoder).
     */
    fun decodeBitmap(stream: InputStream): Bitmap? {
        return try {
            ...
            logcat(LogPriority.ERROR) { "decodeBitmap: ${e.message}" }
            null
        }
    }
    // SY <--
    ...
    fun chooseBackground(context: Context, imageSource: BufferedSource): Drawable {
        val decoder = try {
            ImageDecoder.new(imageSource.inputStream())
        } catch (e: Exception) {
